...
data {
  ...
  int B_PRED; // Number of time points to predict (for AMR)
...
transformed data {
  ...
  vector[B_PRED] TIME_PRED_IDX_STD = (TIME_PRED_IDX - mean(TIME_PRED_IDX)) / sd(TIME_IDX);
  ...
  matrix[B_PRED,M2] PHI2_PRED = PHI(B_PRED, M2, L2, TIME_PRED_IDX_STD);
...
model {
  ...
  { // Local scope
    ...
    target += neg_binomial_lpmf(Y | append_row(mu[M], mu[F]) / theta, inv(theta));
  }
}
...
generated quantities {
  ...
  array[G] matrix[A, B_PRED] MR;
  array[G] vector[B_PRED] AMR;        // Smoothed age-standardised mortality rates

  { // Local scope
    array[G] matrix[A, B_PRED] log_lambda_pred;
    array[G] matrix[A, B_PRED] f2_pred;
    f2_pred[M] = hsgp(A, B_PRED, sigma[M], rho1[M], rho2[M], L1, L2, M1, M2, PHI1, PHI2_PRED, z[1:M1,:]);
    f2_pred[F] = hsgp(A, B_PRED, sigma[F], rho1[F], rho2[F], L1, L2, M1, M2, PHI1, PHI2_PRED, z[(M1+1):2*M1,:]);

    for (a in 1:A){
      if (a == 1){
        log_lambda_pred[M][a,:] = beta0 + f2_pred[M][a,:] + 1e-13;
        log_lambda_pred[F][a,:] = beta0 + f2_pred[F][a,:] + 1e-13;
      } else {
        log_lambda_pred[M][a,:] = beta0 + beta_a[a-1] + f2_pred[M][a,:] + 1e-13;
        log_lambda_pred[F][a,:] = beta0 + beta_a[a-1] + f2_pred[F][a,:] + 1e-13;
      }
    }

    // Mortality rates by age groups
    MR[M] = exp(log_lambda_pred[M]);
    MR[F] = exp(log_lambda_pred[F]);

    // Age-standardised mortality rates
    for (b in 1:B_PRED){
      AMR[M][b] = W * exp(log_lambda_pred[M][:,b]);
      AMR[F][b] = W * exp(log_lambda_pred[F][:,b]);
    }
    ...
    Yhat[M] = neg_binomial_rng(mu[M] / theta, inv(theta));
    Yhat[F] = neg_binomial_rng(mu[F] / theta, inv(theta));

    for (i in 1:N){
      log_lik[i] = neg_binomial_lpmf(Y[i] | mu_long[i] / theta, inv(theta));
    }
  }
}
